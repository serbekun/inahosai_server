...
The site currently talks to no CDN, no webfonts, no analytics. The favicon is generated by the server and served from its own root, with an inline SVG for modern tabs. Visitors of a school site are children; their browsers should not phone home to a third party just to render a page. A YouTube embed on the live page is the one exception, and only when a stream id is configured. Keep it that way.
