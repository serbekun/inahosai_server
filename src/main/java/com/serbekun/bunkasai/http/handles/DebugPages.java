...
public class DebugPages implements HttpHandler {
    ...
    public void register(Javalin svr) {
        ...
        if (!enabled) {
            log.info("/debug playground mount is disabled (debug.enabled=false)");
            return;
        }

        svr.get("/debug/{file}", this::serve);

        if (token.isEmpty()) {
            log.info("/debug playground mount is enabled without a token");
        } else {
            log.info("/debug playground mount is enabled behind a token");
        }
    }
